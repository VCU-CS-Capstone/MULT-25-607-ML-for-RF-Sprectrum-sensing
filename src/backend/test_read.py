...
num_rows = 200
center_freq = 750e6
sample_rates = [50, 40, 20, 10, 5, 2, 1, 0.5]  # MHz
...
class SDRWorker(QObject):
    def __init__(self):
        ...
        self.freq = (
            0  # in kHz, to deal with QSlider being ints and with a max of 2 billion
        )
    ...
    def run(self):
        ...
        self.spectrogram[:] = np.roll(
            self.spectrogram, 1, axis=1
        )  # shifts waterfall 1 row
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setFixedSize(
            QSize(1500, 1000)
        )  # window size, starting size should fit on 1920 x 1080
        ...
        self.sdr_thread.setObjectName(
            "SDR_Thread"
        )  # so we can see it in htop, note you have to hit F2 -> Display options -> Show custom thread names
        worker = SDRWorker()
        worker.moveToThread(self.sdr_thread)

        # Time plot
        time_plot = pg.PlotWidget(
            labels={"left": "Amplitude", "bottom": "Time [microseconds]"}
        )
        ...
        auto_range_button.clicked.connect(
            lambda: time_plot.autoRange()
        )  # lambda just means its an unnamed function
        ...
        freq_plot.setXRange(
            center_freq / 1e6 - sample_rate / 2e6, center_freq / 1e6 + sample_rate / 2e6
        )
        ...
        # Waterfall plot
        waterfall = pg.PlotWidget(
            labels={"left": "Time [s]", "bottom": "Frequency [MHz]"}
        )
        imageitem = pg.ImageItem(
            axisOrder="col-major"
        )  # this arg is purely for performance
        ...
        colorbar.item.gradient.loadPreset(
            "viridis"
        )  # set the color map, also sets the imageitem
        imageitem.setLevels(
            (-30, 20)
        )  # needs to come after colorbar is created for some reason
        ...
        freq_slider.sliderMoved.connect(
            worker.update_freq
        )  # there's also a valueChanged option
        freq_label = QLabel()
        ...
        update_sample_rate_label(
            sample_rate_combobox.currentIndex()
        ...
        def freq_plot_callback(PSD_avg):
            ...
            f = (
                np.linspace(
                    freq_slider.value() * 1e3 - worker.sample_rate / 2.0,
                    freq_slider.value() * 1e3 + worker.sample_rate / 2.0,
                    fft_size,
                )
                / 1e6
            ...
            freq_plot.setXRange(
                freq_slider.value() * 1e3 / 1e6 - worker.sample_rate / 2e6,
                freq_slider.value() * 1e3 / 1e6 + worker.sample_rate / 2e6,
            )
        ...
        worker.time_plot_update.connect(
            time_plot_callback
        ...
        self.sdr_thread.started.connect(
            worker.run
        )  # kicks off the worker when the thread starts
...
)  # this lets control-C actually close the app
